...
export const Avatar: React.FC<AvatarProps> = ({ config, avatarAssets, imageFrame, isEnabledSubtitles, onPlay, initialIsTextOnly }) => {
  ...
  const handleToggleTextOnly = async () => {
    ...
    addToast(newIsTextOnly ? 'Text-only mode activated 💬' : 'Microphone mode activated 🎙️', 'success');
  };
    ...
    <div className="flex flex-col flex-grow w-full relative">
      ...
      >
        <AnimatedSpineCharacter
          ...
          height={imageFrame === 'fullscreen' ? (isMobile ? (hasExperienceStarted ? '30vh' : '65vh') : '53%') : '59vh'}
          width={imageFrame === 'fullscreen' ? (isMobile ? (hasExperienceStarted ? '40vw' : '65vw') : '44vw') : '100%'}
